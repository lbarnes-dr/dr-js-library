...
export default function DrjsWrapper({
    ...
    const createSource = async (element, options) => {
        ...
        const source = await digitalRiver.createSource.apply(digitalRiver, sourceArgs);
        setSource(source);
        returnSource(source);
        return source;
    };
    ...
    useEffect(()=> {
        if (!mounted.current) {
            ...
            if(source && digitalRiver && !sourceRunning) {
                sourceRunning=true;
                digitalRiver.retrieveSource(source.source.id, source.source.clientSecret).then((_source)=>{
                    setSource(_source);
    ...
    return (
        <DrjsContext.Provider value={sharedState}>
            {children}
        </DrjsContext.Provider>
    )
}
